        ...
        for sentence in sentences:
            ...
            for w in sentence:
                if w in model.wv.vocab and model.wv.vocab[w].sample_int > model.random.rand() * 2 ** 32:
                    word = model.wv.vocab[w]
                    word.offset = 0
                    word_vocabs.append(word)
                elif model.model_fixed is not None and w in model.model_fixed.wv.vocab \
                        and model.model_fixed.wv.vocab[w].sample_int > model.model_fixed.random.rand() * 2 ** 32:
                    word = model.model_fixed.wv.vocab[w]
                    word.offset = model.vocab_size
                    word_vocabs.append(word)
            for pos, word in enumerate(word_vocabs):
                if word.offset == 0:
                    term = model.wv.index2word[word.index]
                else:
                    term = model.model_fixed.wv.index2word[word.index]
                ...
                for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start):
                    ...
                    if pos2 != pos:
                        train_sg_pair(
                            model, term, word2.index + word2.offset, alpha, compute_loss=compute_loss
                        )
...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
    ...
    # target word (NN output)
    if word in model.wv.vocab:
        predict_word = model.wv.vocab[word]
        predict_word.offset = 0
    elif word in model.model_fixed.wv.vocab:
        predict_word = model.model_fixed.wv.vocab[word]
        predict_word.offset = model.vocab_size
    else:
        return
    ...
    elif context_index >= model.vocab_size:
        fixed_index = context_index - model.vocab_size
        l1 = model.model_fixed.wv.syn0[fixed_index]
        lock_factor = 0
    else:
...
class Word2Vec(BaseWordEmbeddingsModel):
    ...
    def train(self, sentences=None, corpus_file=None, total_examples=None, total_words=None,
              epochs=None, start_alpha=None, end_alpha=None, word_count=0, window=9,
        ...
        self.window = window
        result = super(Word2Vec, self).train(
